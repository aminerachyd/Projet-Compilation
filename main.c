...
// Déclaration de la structure du symbole courant
// contient le code du symbole ( TOKEN ) et le nom du symbole
typedef struct {
    CODES_LEX CODE;
    char NOM[20];
} TSym_Cour;
...
// Symbole courant, contient son nom ainsi que son token
TSym_Cour Sym_Cour;

int Car_Cour_Sym;

// Fonction pour afficher une erreur
// à partir du tableau d'enumerations des erreurs
...
void Lire_special() {
    ...
    if (!strcmp(Sym_Cour.NOM, "{*")) { // Traitement des commentaires
        while (1) {
            Car_Cour = fgetc(file);
            if (Car_Cour == 42) {
                memset(Sym_Cour.NOM, 0, 20);
...
void Test_Symbole(CODES_LEX CODE, ERREURS CODE_ERR) {
    while (Sym_Cour.CODE == NULL_TOKEN) {
        Sym_Suiv();
    }
...
void ID() {
    Lettre();
    // Le premier caractère du symbole étant une lettre
    // On teste si le caractère suivant est soit une lettre soit un chiffre
    while (((Sym_Cour.NOM[Car_Cour_Sym] > 64 && Sym_Cour.NOM[Car_Cour_Sym] < 91) ||
           ...
           (Sym_Cour.NOM[Car_Cour_Sym] > 48 && Sym_Cour.NOM[Car_Cour_Sym] < 57)) {
...
void NUM() {
    Chiffre();
    while ((Sym_Cour.NOM[Car_Cour_Sym] > 48 && Sym_Cour.NOM[Car_Cour_Sym] < 57)) {
        Chiffre();
    }
    Sym_Suiv();
}
...
void Chiffre() {
    // Test si le premier caractère du symbole est un chiffre
    if (Sym_Cour.NOM[Car_Cour_Sym] > 48 && Sym_Cour.NOM[Car_Cour_Sym] < 57) {
        Car_Cour_Sym++;
        return;
    }
...
void Lettre() {
    // Test si le premier caractère du symbole est une lettre
    if ((Sym_Cour.NOM[Car_Cour_Sym] > 64 && Sym_Cour.NOM[Car_Cour_Sym] < 91) ||
        (Sym_Cour.NOM[Car_Cour_Sym] > 96 && Sym_Cour.NOM[Car_Cour_Sym] < 123)) {
        Car_Cour_Sym++;
        return;
